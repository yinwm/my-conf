...
;; 
;; Exported names start with "js-" whereas private names start
;; with "js-".
;; 
;; Changes:
;;
...
(when js-auto-indent-flag
  (mapc (lambda (key) 
	  (define-key js-mode-map key 'js-insert-and-indent))
	'("{" "}" "(" ")" ":" ";" ",")))
            ...
            ((nth 1 parse-status)
             ...
             (if (looking-at "[({[][ \t]*\\(/[/*]\\|$\\)")
               ...
               (unless same-indent-p
                 ...
                 (skip-chars-forward " \t"))
               (current-column)))
	    (continued-expr-p js-indent-level)
            (t 0)))))
